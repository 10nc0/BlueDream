...
async function addGroupMessagesColumn() {
    ...
    try {
        ...
        for (const schema of tenantSchemas) {
            try {
                ...
                if (!hasBooksTable.rows[0].exists) {
                    console.log('  ' + schema + ': No books table found (skipping)');
                ...
                if (hasColumn.rows[0].exists) {
                    console.log('  ' + schema + ': Column already exists (skipping)');
                ...
                if (DRY_RUN) {
                    ...
                    await pool.query('ALTER TABLE ' + schema + '.books ADD COLUMN include_group_messages BOOLEAN DEFAULT false');
        ...
        if (totalErrors > 0) {
            console.log('Some schemas had errors. Review the output above.\n');
            process.exit(1);
        }
