...
class PhiBreathe {
    ...
    subscribe(name, intervalMs, callback) {
        ...
        logger.info({ subscriber: name, cycleMinutes: Math.round(intervalMs / 1000 / 60) }, 'Phi breathe: subscriber registered');
    }
    ...
    async startPhiBreathe() {
        ...
        // Production detection first — dev always runs locally, skips leader election.
        // Replit sets REPLIT_DEPLOYMENT=1 when deployed; also accept NODE_ENV=production.
        const isProd = process.env.REPLIT_DEPLOYMENT === '1' || process.env.NODE_ENV === 'production';
        this.isProduction = isProd;

        if (!isProd) {
            // Dev: skip advisory lock (prod instance holds it on shared DB), run locally.
            this.phiBreatheCount = 0;
        ...
        if (this.pool) {
            ...
            if (!lockRes.rows[0].pg_try_advisory_lock) {
                logger.debug('💤 Phi breathe: standby (another instance is leader)');
                return;
            }
    ...
    async _atomicPhiBreathe() {
        try {
            ...
            const emoji = isInhale ? '🌬️' : '💨';
            logger.info({ count: this.phiBreatheCount, type, durationMs: duration }, `${emoji} Phi #${this.phiBreatheCount} ${type}${multiplier}`);
    ...
    _devPhiBreathe() {
        ...
        const emoji = isInhale ? '🌬️' : '💨';
        logger.info({ count: this.phiBreatheCount, type, durationMs: duration, mode: 'dev' }, `${emoji} Phi #${this.phiBreatheCount} ${type}${multiplier}`);
