...
class PhiBreathe {
    ...
    async startPhiBreathe() {
        ...
        if (!isProd) {
            // Dev breathe is intentionally ephemeral — resets on every restart,
            // never written to DB, never embedded in ledger messages (queue
            // processor is disabled in dev).  Keeps dev completely isolated from
            // the prod append-only sequence.
            this.phiBreatheCount = 0;
