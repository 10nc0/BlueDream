...
class PhiBreathe {
    ...
    deregisterNode(name) {
        if (this.nodes.has(name)) {
            ...
            logger.info({ node: name }, '🔌 NyanMesh: node deregistered — %s', name);
        ...
        try {
            ...
            await this.pool.query(
                `INSERT INTO core.system_counters (key, value, updated_at)
                 VALUES ('mesh_node_registry', $1, NOW())
