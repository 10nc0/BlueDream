...
class PhiBreathe {
    ...
    async startPhiBreathe() {
        ...
        // Dev = NODE_ENV is 'development' or empty (Replit workspace default)
        // Prod = NODE_ENV is explicitly 'production' (set by deploy config)
    ...
    async orchestrateStartup() {
        ...
        // 4. SHARE INVITE TIMEOUT: Immediate + 24h cycle - expire 7-day old unregistered invites
        await this._expireStaleShareInvites();
        this.subscribe('share-invite-cleanup', 24 * 60 * 60 * 1000, async () => {
            await this._expireStaleShareInvites();
        });
    ...
    async _expireStaleShareInvites() {
        try {
            ...
            // Find invites older than 7 days where the invitee hasn't registered
            ...
            const expiredResult = await this.pool.query(`
                ...
                WHERE bs.revoked_at IS NULL
                  AND bs.invited_at < NOW() - INTERVAL '7 days'
                  AND NOT EXISTS (
