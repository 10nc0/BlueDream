...
class PhiBreathe {
    ...
    async startPhiBreathe() {
        ...
        // LEADER ELECTION: Only one process/instance should run phi breathe.
        // pg_try_advisory_lock is session-scoped — auto-releases when the connection
        // closes (process dies), so no TTL bookkeeping needed.
        ...
        if (this.pool) {
            ...
            if (!lockRes.rows[0].pg_try_advisory_lock) {
                console.log('🌬️  phi breathe: standby (another instance is leader)');
                return;
            }
            console.log('🌬️  phi breathe: acquired leader lock — this instance is the heartbeat');
        }
    ...
    _devPhiBreathe() {
        ...
        console.log(`🌬️  phi breathe #${this.phiBreatheCount} ${type} ${duration}ms${multiplier} (dev)`);
    ...
    async orchestrateStartup() {
        ...
        // 5. PENDING BOOK EXPIRY: Immediate + 24h cycle - expire books never activated in 7 days
    ...
    async _expirePendingBooks() {
        try {
            console.log('⏰ Phi breathe: Starting 72h unlinked book expiry...');
            ...
            if (expiredResult.rowCount > 0) {
                console.log(`⏰ Expired ${expiredResult.rowCount} pending book(s) (72h timeout, never activated)`);
                const preview = expiredResult.rows.slice(0, 5).map(r => `${r.book_name} (${r.fractal_id})`);
                console.log(`   Expired books: ${preview.join(', ')}${expiredResult.rowCount > 5 ? '...' : ''}`);
            } else {
                console.log('✅ No hanging pending books to expire');
            }
        } catch (error) {
            console.error('❌ Pending book expiry failed:', error.message);
        }
    }
