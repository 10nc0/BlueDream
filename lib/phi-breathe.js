...
class PhiBreathe {
    ...
    registerNode(name, meta = {}) {
        ...
        logger.info({ node: name, meta }, '🔌 NyanMesh: node registered — %s', name);
    ...
    async _snapshotNodeRegistry() {
        ...
        try {
            // Store per-node online/offline flags using existing BIGINT value column.
            // Key pattern: mesh_node_{name} → 1 (online) or 0 (offline). No schema change needed.
            const pairs = [];
            for (const [name, entry] of this.nodes) {
                pairs.push(`('mesh_node_${name}', ${entry.status === 'online' ? 1 : 0})`);
            }
