...
class PhiBreathe {
    ...
    async startPhiBreathe() {
        ...
        // Retry loop: during deployment rollover the old instance may still hold
        // the lock for a few seconds until its connection closes.
        if (this.pool) {
            ...
            for (let attempt = 1; attempt <= MAX_LOCK_RETRIES; attempt++) {
                ...
                logger.warn({ attempt, maxRetries: MAX_LOCK_RETRIES }, '💤 Phi breathe: leader lock held by another instance, retrying in %dms…', LOCK_RETRY_DELAY_MS);
                await new Promise(r => setTimeout(r, LOCK_RETRY_DELAY_MS));
            }

            if (!acquired) {
                logger.warn('⚠️ Phi breathe: could not acquire leader lock after %d attempts — standing by', MAX_LOCK_RETRIES);
                return;
            }
