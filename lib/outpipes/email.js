...
// ── Email = a distinct transport, NOT a webhook ───────────────────────────────
//
// Email does not POST a payload to a URL — it calls the Resend API and builds
// MIME (subject / html / attachments). The transport is different, so this
// extends BaseOutpipe directly rather than WebhookOutpipe. (SMS via Twilio
// would follow the same pattern: different transport → extend BaseOutpipe.)
