...
const { CODE_EXTENSIONS } = require('../utils/file-types');
...
const MODE_DEFINITIONS = {
  ...
  'seed-metric': {
    ...
    description: 'Web-first (Brave → DDG cascade) for metro-level $/m² and income data. Users rarely have this data locally.',
    priority: 20,
    personality: {
      skipIntroOutro: true,  // Preserve structured Fatalism/Optimism/Extraction readings
      preserveTechnicalTerms: true,
      appendSignature: true,
      signatureText: SIGNATURE_SPECIALIZED
    }
  ...
  'code-audit': {
    ...
    description: 'Professional security auditor for code. Detects snippets in query OR uploaded files.',
    priority: 30,
    personality: {
      skipIntroOutro: true,
      preserveTechnicalTerms: true,
