...
const schemas = {
    ...
    createDrop: z.object({
        book_id: z.string().min(1, 'Book ID required').regex(BOOK_ID_PATTERN, 'Invalid book ID format'),
        discord_message_id: z.string().min(1, 'Discord message ID required').max(100, 'Message ID too long'),
        metadata_text: z.string().min(1, 'Metadata text required').max(10000, 'Metadata text too long')
    }),

    deleteDropTag: z.object({
        book_id: z.string().min(1, 'Book ID required').regex(BOOK_ID_PATTERN, 'Invalid book ID format'),
        discord_message_id: z.string().min(1, 'Discord message ID required').max(100, 'Message ID too long'),
        tag: z.string().min(1, 'Tag required').max(500, 'Tag too long')
    }),

    deleteDropDate: z.object({
        book_id: z.string().min(1, 'Book ID required').regex(BOOK_ID_PATTERN, 'Invalid book ID format'),
        discord_message_id: z.string().min(1, 'Discord message ID required').max(100, 'Message ID too long'),
        date: z.string().min(1, 'Date required').max(100, 'Date too long')
    }),
