...
function createSendToLedger(pool, fallbackWebhookUrl) {
    return async function sendToLedger(payload, options = {}, book = null) {
        ...
        try {
            ...
            // Chunk content at Discord's 2 000-char webhook limit.
            // Media options pass through on the first chunk only; subsequent
            // chunks are plain text so Discord doesn't reject them for size.
            const chunks = splitMessageIntoChunks(payload.content || '', MESSAGE_CHUNK_SIZE <= 2000 ? MESSAGE_CHUNK_SIZE : 2000);
            let response;
            for (let i = 0; i < chunks.length; i++) {
                const chunkPayload = { ...payload, content: chunks[i] };
                const chunkOptions = i === 0
                    ? { mediaBuffer: options.mediaBuffer, mediaFilename: options.mediaFilename, mediaContentType: options.mediaContentType, isMedia: options.isMedia, mediaBufferId: options.mediaBufferId, tenantSchema: options.tenantSchema, pool }
                    : { tenantSchema: options.tenantSchema, pool };
                response = await postPayloadToWebhook(url.toString(), chunkPayload, chunkOptions);
            }

            const _deliveredTo = (output?.type === 'thread' && output?.thread_id)
                ? `thread:${output.thread_id}`
                : (output?.channel_id ? `channel:${output.channel_id}` : 'channel');
            logger.info({ deliveredTo: _deliveredTo, chunks: chunks.length }, '📌 Output #01 (Ledger) delivered');
