...
async function assertPublicUrl(url) {
    ...
    // Reject bare localhost regardless of DNS
    ...
    try {
        ...
        if (err.message === 'URL resolves to a private or reserved IP address') throw err;
        // DNS failure (NXDOMAIN etc.) — let the fetch fail naturally with a clear error
        logger.warn({ hostname, err: err.message }, 'SSRF guard: DNS lookup failed');
    }
}
