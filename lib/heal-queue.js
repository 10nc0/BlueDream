...
class HealQueue {
    ...
    async healSingleBook(book) {
        try {
            ...
            const _existingLedger = resolveOutput(tenantBook, 'output_01');

            if (_existingLedger?.thread_id) {
                await this.pool.query(`
                    UPDATE core.book_registry
                    SET heal_status = 'healthy',
                        heal_attempts = 0,
            ...
            if (dualThreads.output_01?.thread_id) {
                ...
                phiBreathe.emit('heal:complete', { bookId: book.fractal_id, channelId: dualThreads.output_01.channel_id });
            } else {
