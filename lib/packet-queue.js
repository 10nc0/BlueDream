...
async function processWebhookMessage(msg, rawPayload, deps) {
    ...
    //
    // OPS NOTE: archival is intentionally non-blocking — the queue item is marked done before
    // this resolves. On abrupt process termination (SIGKILL mid-pin, OOM), the ledger+archive
    // writes may be skipped. Discord (via sendToLedger above) remains the synchronous durable
    // path for webhook messages. Consider zero-downtime rolling redeploys to minimise this window.
    processCapsule(
        book,
