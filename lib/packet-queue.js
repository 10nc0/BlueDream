...
async function routeMessage(pool, msg, logger) {
    ...
    if (msg.joinCode) {
        ...
        // TRUST: this branch is reached only by phone-bearing channels (twilio).
        // book_engaged_phones stores carrier-verified E.164 numbers — Telegram and
        // LINE route via channel_identifiers above; webhook short-circuits before
        // routeMessage is called. A future non-phone channel with no join code
        // would fall here and find nothing (phone = null → empty result), which
        // routes to limbo — the safe failure mode.
...
//   false — original behaviour: entire pipeline is fire-and-forget; the caller
//           returns before any DB write completes. Used by the active-book path
//           where Discord is the synchronous durable path.
//   true  — DB writes (core.message_ledger + anatta_messages) are awaited before
//           the function resolves, so the queue item is only marked done after both
//           rows are committed. IPFS pin + CID back-fill remain fire-and-forget
//           (they are optional and network-dependent). Used by processWebhookMessage
//           so the PostgreSQL read path is durable relative to the queue.
async function processCapsule(book, bookRecord, msg, media, discordResponse, deps, opts = {}) {
                ...
                await pool.query(
                    format(
                        `INSERT INTO %I.anatta_messages
                         (book_fractal_id, message_fractal_id, sender_name, body, has_attachment, media_url, phi_breathe_stamp, recorded_at)
    ...
    if (awaitDbWrites) {
        // Webhook / node-to-node path: DB rows committed before queue item is done.
        try {
            await writeDbRows();
        } catch (err) {
            logger.warn({ err: err.message }, '⚠️ Capsule DB write error (non-fatal)');
        }
