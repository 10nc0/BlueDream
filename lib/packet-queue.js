...
async function processWebhookMessage(msg, rawPayload, deps) {
    ...
    // Resolve thread credentials — nested (output_01.thread_id) takes priority over
    // legacy flat (thread_id) to handle books activated before the nested schema.
    const _oc01 = book.output_credentials?.output_01;

    const sendToLedger = helpers?.sendToLedger;
    if (sendToLedger) {
        await sendToLedger(discordPayload, {
            isMedia: !!msg.media_url,
            threadName: _oc01?.thread_name || book.output_credentials?.thread_name || null,
            threadId: _oc01?.thread_id || book.output_credentials?.thread_id || null
        }, book);
    }
                ...
                await pool.query(
                    format(
                        `INSERT INTO %I.anatta_messages
                         (book_fractal_id, message_fractal_id, sender_name, body, has_attachment, media_url, media_type, phi_breathe_stamp, recorded_at)
