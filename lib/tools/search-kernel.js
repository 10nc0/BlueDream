...
const TIERS = {
  ...
  // Generic live-web research only. Seed Metric deliberately stays on premium
  // so its structured sources and legacy Brave → DDG fallbacks do not change.
  generic:  ['tinyfish', 'brave', 'ddg'],
};
...
class SearchKernel {
  ...
  _load() {
    ...
    if (!this._providers.tinyfish) this._providers.tinyfish = require('./tinyfish-search');
  }

  _keyOk(provider) {
    if (provider === 'brave') return !!process.env.PLAYGROUND_BRAVE_API;
    if (provider === 'exa')   return !!process.env.EXA_API_KEY;
    if (provider === 'tinyfish') return !!process.env.TINYFISH_API_KEY;
    return true;
  }
  ...
  async _callProvider(providerName, query, clientIp, format, domainType, providerOptions = {}) {
    ...
    if (providerName === 'tinyfish') {
      const opts = { ...providerOptions, domainType };
      if (format && format !== 'text') opts.format = format;
      return p.execute(query, opts);
    }
   ...
   * @param {string}  [opts.tier='standard']   free | standard | premium | semantic | generic
   * @param {string}  [opts.clientIp=null]
   * @param {string}  [opts.format='text']     'text' | 'json'
   * @param {string}  [opts.domainType='web']  web | news | research_paper
   * @param {object}  [opts.providerOptions]   Provider-specific search filters
   * @returns {Promise<{ result: string|null, provider: string|null, tier: string, latencyMs: number }>}
   */
  async search({ query, tier = 'standard', clientIp = null, format = 'text', domainType = 'web', providerOptions = {} } = {}) {
    if (!query || typeof query !== 'string' || !query.trim()) {
      return { result: null, provider: null, tier, latencyMs: 0 };
    }

    this._load();
    const resolvedDomainType = TINYFISH_DOMAIN_TYPES.has(domainType) ? domainType : 'web';
    ...
    for (const providerName of chain) {
      ...
      const result = await this._callProvider(providerName, query, clientIp, format, resolvedDomainType, providerOptions);
...
module.exports = {
  ...
  name: 'search-kernel',
  description: 'Tiered search kernel — TinyFish, DDG, Brave, and Exa behind a unified interface. Tier selects provider chain by use case and availability.',
  parameters: {
    query: { type: 'string', required: true,  description: 'Search query' },
    tier:  { type: 'string', required: false, description: "'free' | 'standard' | 'premium' | 'semantic' | 'generic'" }
  },
