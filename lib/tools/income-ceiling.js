...
async function _fetchAnchorGni(iso2) {
    ...
    return urlCache.coalesce(cacheKey, async () => {
...
async function getIncomeCeilingUsd() {
    return urlCache.coalesce('income-ceiling:usd', async () => {
...
async function getIncomeCeilingLcu(currency) {
    ...
    if (code === 'USD') return getIncomeCeilingUsd();
    ...
    if (!iso2) {
        ...
        return getIncomeCeilingUsd();
    }

    return urlCache.coalesce(`income-ceiling:lcu:${code}`, async () => {
