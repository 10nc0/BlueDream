...
class EmailChannel extends BaseChannel {
    ...
    parsePayload(req) {
        ...
        // Extract display name from "Display Name <email@host>" format
        const displayNameMatch = rawFrom.match(/^(.+?)\s*</);
        const displayName = displayNameMatch ? displayNameMatch[1].trim() : null;

        return { from, rawFrom, displayName, toAddress, toLocal, body: fullBody, messageId };
    }
    ...
    normalizeMessage(rawPayload) {
        return {
            ...
            senderName: rawPayload.displayName || rawPayload.from,
