...
class TwilioChannel extends BaseChannel {
    ...
    validateSignature(req) {
        ...
        const constructedUrl = `${protocol}://${host}${req.originalUrl}`;
        
        const urlVariations = [
            process.env.TWILIO_WEBHOOK_URL,
            constructedUrl,
            constructedUrl.replace(/\/$/, ''),
            constructedUrl.endsWith('/') ? constructedUrl : constructedUrl + '/',
            `https://${host}${req.originalUrl}`,
            `https://${host}${req.originalUrl}`.replace(/\/$/, '')
        ].filter(Boolean);
        ...
        for (const url of uniqueUrls) {
            ...
            if (isValid) {
                this.logger.info({ validatedUrl: url }, 'Twilio signature validated');
                return { valid: true };
            }
        }
        ...
        }, 'Twilio signature validation failed for all URL variations');
