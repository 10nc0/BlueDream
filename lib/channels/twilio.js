...
class TwilioChannel extends BaseChannel {
    ...
    async downloadMedia(mediaUrl, mediaContentType) {
        ...
        try {
            logger.info({ mediaUrl: mediaUrl.substring(0, 60) }, 'Downloading media from Twilio');
            ...
            logger.info({ bytes: buffer.length, contentType }, 'Downloaded media');
            
            return { buffer, filename, contentType };
        } catch (error) {
            logger.error({ error: error.message, mediaUrl: mediaUrl.substring(0, 60) }, 'Failed to download media');
            return null;
        }
    }
    ...
    async sendReply(to, message) {
        ...
        try {
            ...
            logger.info({ to }, 'Sent Twilio reply');
            return true;
        } catch (error) {
            logger.warn({ error: error.message, to }, 'Could not send Twilio reply');
            return false;
        }
    }
