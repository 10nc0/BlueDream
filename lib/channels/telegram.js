...
class TelegramChannel extends BaseChannel {
    ...
    normalizeMessage(rawPayload) {
        ...
        return {
            ...
            senderName:       rawPayload.displayName || String(rawPayload.userId),
    ...
    async downloadMedia(fileId, contentType) {
        ...
        try {
            // Step 1: resolve file path via getFile (Telegram-specific two-step)
            ...
            // Derive extension from filePath so the filename is meaningful
            // regardless of what the Content-Type header says.
            const ext = filePath.split('.').pop() || 'bin';

            // Step 2: download binary via resolveBuffer
            ...
            this.logger.info({ bytes: result.buffer.length, resolvedType: result.mimeType }, 'Telegram media downloaded');
