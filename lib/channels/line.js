...
class LineChannel extends BaseChannel {
    ...
    parsePayload(req) {
        ...
        return {
            ...
            hasMedia: isMedia,
            // mediaUrl is the LINE message ID — downloadMedia() uses it as the Content API key
            mediaUrl: isMedia ? msg.id : null,
    ...
    normalizeMessage(rawPayload) {
        ...
        return {
            ...
            // LINE webhooks carry no display name — userId is the stable channel identity.
            // Enrich with a Profile API call in a future pass when display names are needed.
            senderName: rawPayload.userId,
    ...
    async downloadMedia(messageId, contentType) {
        ...
        try {
            ...
            const result = await resolveBuffer({
                mediaUrl: `https://api-data.line.me/v2/bot/message/${messageId}/content`,
                axiosOpts: { headers: { Authorization: `Bearer ${this.accessToken}` } },
                mimeType: contentType || null,
                prefixName: 'line'
            });
            if (!result) return null;
            this.logger.info({ bytes: result.buffer.length, resolvedContentType: result.mimeType }, 'Line media downloaded');
