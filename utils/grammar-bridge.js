/**
 * Grammar Bridge — S-1.5
 *
 * Translates the grammar of asking (raw natural language) into the grammar
 * of routing (structured intent + context + lens). Sits between S-1 (context
 * extraction) and S0 (preflight routing).
 *
 * The bridge is a tool of the agent, not an agent itself. It does not predict
 * ground sources, fetch scrolls, or reason about what the answer needs — it
 * translates. Reasoning about what to fetch belongs to the LLM (handled
 * separately via tool-calling).
 *
 * BridgeResult schema:
...
// ==================== Main export ====================
/**
 * bridgeQuery — S-1.5 grammar bridge translation
 *
 ...
 */
async function bridgeQuery(rawQuery, sessionLens = {}, groqToken = null) {
  ...
  try {
    ...
    }, '🔍 S-1.5 Grammar Bridge result');

    return result;

  } catch (err) {
    logger.warn({ err: err.message }, '⚠️ S-1.5 Grammar Bridge model call failed — using safe default');
