...
-- Per-user onboarding state. user_id is intentionally the primary key:
-- the onboarding route upserts exactly one settings record per user.
...
CREATE TABLE IF NOT EXISTS ${SCHEMA}.books (
    ...
    monthly_email_backup BOOLEAN NOT NULL DEFAULT TRUE,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
