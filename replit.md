...
- **WhatsApp Integration**: Twilio-based messaging integration using WhatsApp Business API. **Sybil-proof activation** via unique join codes (format: "join baby-ability BOOKNAME-abc123"). Each book generates a 6-char random code (16.7M combinations) stored in registry with status='pending'. When user sends the code via WhatsApp, single registry query finds book (O(1)), activates it (status='active', sets phone_number), and maps phone to tenant. **Case-preserved parsing** ensures "GW-37d0f6" matches database exactly. Webhook routing: phone → core.phone_to_tenant → tenant_schema → registry lookup → book_id → Discord.
