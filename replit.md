...
- **Mobile/Desktop parity**: Whenever making any UI size, spacing, or layout change, always check BOTH the desktop CSS rule AND the `@media (max-width: 480px)` mobile override — they are independent `!important` blocks and a fix to one silently leaves the other broken. Auth pages: `cat-animation.css` has separate desktop (200px) and mobile (240px) canvas overrides; canvas buffer is 250px on all 4 auth pages. Dashboard: `mobile-mode` / `desktop-mode` body classes are set by JS `LayoutController`. **Cat architecture (AUTHORITATIVE)**: `#catContainer` uses `position: fixed; top: 0; left: 0` — pinned to absolute top-left of viewport, completely outside all flex/grid layout so it never disrupts the header. `cat-animation.css` (loaded on `index.html`) owns canvas sizing: desktop=190px, mobile=80px via `body.desktop-mode .character-canvas` and `body.mobile-mode .character-canvas` rules. `LayoutController` sets canvas drawing buffer: desktop=190×190, mobile=75×75 (inline style is cleared so CSS !important wins display size). Header has `padding-left: 190px` (desktop) / `80px` (mobile) to push the title text to the right of the cat. Cat canvas HTML attrs: `width="190" height="190"`. Drawing scale = `(canvas.width / 125) × 2.8`. Cat face (ears-to-chin) appears at canvas y≈52–112; blank canvas above y=52 is transparent. z-index: var(--z-modal-above) (1001) on BOTH modes — transcendental layer above all UI. `pointer-events: none` so header buttons remain clickable through the cat.
