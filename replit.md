...
- **Unified Shadow QR Flow**: Both Create Bridge and Generate QR use shadow sessions to prevent storage bloat from abandoned connections. Bridges remain inactive until WhatsApp connection succeeds, then shadow promotes to primary. Warning modal shows ONLY when replacing active inpipe (connected bridges), not for new bridges.
- **Zero-Downtime Reconnection**: Shadow session architecture allows reconnecting WhatsApp without message loss. Primary session stays active while shadow authenticates, then atomic swap promotes shadow to primary. Discord threads/webhooks remain unchanged. 5-minute auto-cleanup for abandoned shadows.
