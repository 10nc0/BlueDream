...
- **Floating Thumbs Zone**: Bottom-right pill bar with 48×48px buttons in standardized layout:
  ...
  - Position 4: Bridge Info (📋, green border, ALWAYS shows current bridge with QR relink/Edit/Delete)
  - Position 5: Bridge Card (🔗, slate border) - shows Fan modal if 4+ bridges, Create form if 0 bridges
  - Position n (leftmost): Next Bridge (→, pink border, navigate forward, only shown if 2+ bridges)
  ...
  - **Compressed Toolbar**: Message toolbar reduced by 40% vertical height with squeezed search box (80px) - all controls (All checkbox, Search, Status filter, Export button) fit without horizontal overflow
