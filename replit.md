...
- **Dashboard**: SPA with Apple glassmorphism design
...
6. **Web Dashboard**: Professional UI with real-time updates
...
- **Security**: API routes still protected with `requireAuth` middleware (supports both JWT and cookies)
...
- **audit_logs**: Session creation, authentication events

### Key Constraints
- Write-only: No DELETE operations (permanent retention)
- Foreign keys: messages.bot_id → bots.id (CASCADE)
...
- **Retention**: Permanent storage (no deletion)
...
4. Test attachment preview (click 📎 icon)
5. Test bot editing and QR login
