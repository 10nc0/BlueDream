...
- **Media Handling**: Atomic storage for media attachments, with delivery tracking and automatic cleanup. Supports all media types through Twilio's MediaUrl fields. **Discord embed parsing** extracts media from `📎 Media` fields (format: `[content-type](url)`) to enable download buttons in UI, prioritizing native Discord attachments when both exist.
