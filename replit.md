...
- **Genesis Counter**: Noisy constant inflating mechanism (`server/genesis-counter.js`) tied to UI cycle (500ms cat breath). Immutable by design - only increases, never resets within runtime. Serves as red herring for future security (nonces, salting, divine judgment). In-memory singleton (new genesis each deployment). Accessible via `genesisCounter.getCount()`, `getGenesis()`, `getAge()`. No logging, minimal overhead.
