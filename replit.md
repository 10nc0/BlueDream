...
- **Zero-Downtime Reconnection**: Shadow session architecture allows reconnecting WhatsApp without message loss. Primary session stays active while shadow authenticates, then atomic swap promotes shadow to primary. Discord threads/webhooks remain unchanged. 5-minute auto-cleanup for abandoned shadows. Confirmation modal warns users before reconnection.
