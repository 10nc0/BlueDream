...
- **Nyan API v1**: Internal JSON API (`POST /api/v1/nyan`) for agent-to-agent communication (e.g., OpenClaw integration). Bearer token auth via `NYAN_API_TOKEN` env var. Returns structured JSON with AI response, mode, badge, confidence, and processing time. For Psi-EMA queries, includes structured `psiEma` object with daily/weekly readings (θ, z, R, fidelity). Health check at `GET /api/v1/nyan/health`. Rate limited to 30 req/min.
