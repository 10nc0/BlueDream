...
- **Cat Pulse Blockchain**: Universal root timestamp system (`server/cat-pulse-blockchain.js`) that logs server "breathes" as blockchain-style incrementing blocks. Synchronized with cat animation pulse (500ms cycle), it provides an immutable reference clock for the entire system. Exposed via `/api/cat-pulse` public endpoint. Logs milestone blocks (every 100 blocks = ~50s) with formatted uptime. Features: singleton architecture, graceful lifecycle management, real-time block height API.
- **φ-Breath System (Mobile)**: Golden ratio breathing animation system (`public/js/ui/phi-breath.js`) for mobile UI only. Constant breath cycle from φ^0 (1.0x = 4000ms) to φ^1 (1.618x ≈ 6472ms). Features yin-yang button rotation synchronized with breath cycle at idle, and fast "creation spins" (1s rotation) when singularity button expands. Event-based architecture with breath logging and duplicate prevention guards.
